...
export function checkTarget(ns, server, target = "n00dles", forms = false) {
    ...
    if (serverSim.requiredHackingSkill <= player.skills.hacking / (forms ? 1 : 2)) {
        if (forms) {
            ...
            currentScore =
                (serverSim.moneyMax / ns.formulas.hacking.weakenTime(serverSim, player)) *
...
// A function to gain root access to the server
/** @param {import("../..").NS} ns */
export function getRootAccess(ns, server) {
    ...
    if (portsOpen >= ns.getServerNumPortsRequired(server)) ns.nuke(server);
}
