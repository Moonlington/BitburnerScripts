...
async function solverWaysToExpress(ns, arrayData) {
	...
	for (i = 0; i < tempPermutations; i++) {
		...
		if (!Boolean(i % 100000)) {
			ns.print(i + "/" + tempPermutations + ", " + validExpressions.length + " found.");
			await ns.sleep(100);
		}
...
export async function main(ns) {
	...
	ns.print("Completed server probe; now solving contracts");
		...
		for (let z = 0; z < listFiles.length; z++) {
			...
			switch (inputType) {
				case "Algorithmic Stock Trader I":
					...
					ns.print([listServers[listIndex],
					listFiles[z],
						inputType,
						outputData,
						outputResult
					...
					if (!outputResult) {
						ns.print("Failed data for debug: " + JSON.stringify(inputData));
					}
					break;
