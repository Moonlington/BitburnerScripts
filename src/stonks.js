import { PrintTable, DefaultStyle } from 'lib/tables.js'
...
export async function main(ns) {
	...
	while (true) {
		...
		if (TIME_TRACKING && performance.now() - started > BENCH_TIME) {
			...
			ns.tprint('FAIL: It\'s been an hour and the current balance is $' + ns.formatNumber(ns, balance) + ' initial funds were $' + ns.formatNumber(ns, initialFunds) + ' profit: ' + ((1 - balance / initialFunds * 100)).toFixed(2) + '%');
